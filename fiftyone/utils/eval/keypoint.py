...
class KeypointEvaluationConfig(DetectionEvaluationConfig):
    def __init__(
        self,
        pred_field,
        gt_field,
...
class KeypointEvaluation(DetectionEvaluation):
    ...
    def generate_results(
        ...
        In this case, a :class:`AsdDetectionResults` instance is returned that
...
class KeypointResults(DetectionResults):
...
def _asd_evaluation(gts, preds, eval_key, config):
    iou_thresh = min(config.iou, 1 - 1e-10)
    id_key = "%s_id" % eval_key
    iou_key = "%s_iou" % eval_key

    cats, pred_ious, iscrowd = _asd_evaluation_setup(
...
def _asd_evaluation_setup(
    ...
    for objects in cats.values():
        ...
        ious = compute_key_point_dist(preds, gts, **iou_kwargs)
...
def _parse_config(pred_field, gt_field, method, is_temporal, **kwargs):
    return KeypointEvaluationConfig(pred_field, gt_field, **kwargs)
